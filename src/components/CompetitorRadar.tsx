                ...
                {selectedCompetitor ? (
                    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="flex items-start justify-between mb-2">
                            <h5 className="font-bold text-white text-sm line-clamp-2">{selectedCompetitor.name}</h5>
                            <span className={cn(
                                "text-[10px] px-2 py-0.5 rounded-full font-black uppercase",
                                selectedCompetitor.type === "THREAT" ? "bg-red-500/20 text-red-400" :
                                    selectedCompetitor.type === "OPPORTUNITY" ? "bg-emerald-500/20 text-emerald-400" : "bg-yellow-500/20 text-yellow-400"
                            )}>{selectedCompetitor.type}</span>
                        </div>

                        <div className="space-y-3 mt-4">
                            <div className="bg-black/20 p-2 rounded-lg">
                                <div className="text-[10px] text-white/40 uppercase mb-1">PUAN & YORUM</div>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-xl font-bold text-white">{selectedCompetitor.rating}</span>
                                    <span className="text-xs text-white/60">({selectedCompetitor.user_ratings_total})</span>
                                </div>
                            </div>

                            <div className="text-xs text-white/70">
                                {selectedCompetitor.type === "THREAT" && (
                                    <p className="flex items-start gap-2">
                                        <AlertTriangle className="w-3 h-3 text-red-500 mt-0.5 shrink-0" />
                                        Güçlü bir rakip. Müşteri sadakati yüksek. Doğrudan rekabet etmek yerine niş alanlara yönelin.
                                    </p>
                                )}
                                {selectedCompetitor.type === "OPPORTUNITY" && (
                                    <p className="flex items-start gap-2">
                                        <CheckCircle2 className="w-3 h-3 text-emerald-500 mt-0.5 shrink-0" />
                                        Zayıf nokta tespit edildi. Müşterileri hizmet kalitesinden memnun değil.
                                    </p>
                                )}
                            </div>
                        </div>
